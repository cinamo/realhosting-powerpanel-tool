...
class DnsAddCommand extends PowerPanelCommand
{
    protected function configure()
    {
        $this
            ->setName('dns:add')
            ->setDescription('Add a DNS record for a domain')
            ->addArgument(
                'domain',
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $result = $this->client->addDnsRecord($domain, $name, $type, $content);

        if($result === true) {
            $output->writeln("DNS record added successfully.");
        } else {
